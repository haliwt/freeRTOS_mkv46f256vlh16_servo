...
static void vTaskUSART(void *pvParameters)
{
 ...
 // const TickType_t xFrequency = 300;
  ...
  const TickType_t xMaxBlockTime = pdMS_TO_TICKS(500); /* 锟斤拷锟斤拷锟斤拷锟饺达拷时锟斤拷为300ms */
  /* 锟斤拷始锟斤拷锟结构锟斤拷指锟斤拷 */
	ptMsg = &g_tMsg;
  ...
  while(1)
    {
		...
		if(ptMsg->usData[0]== 0x3)/*锟斤拷锟斤拷PID 锟斤拷锟斤拷*/
        {
           
	      xTaskNotify(xHandleTaskSUBJ,      /* 目锟斤拷锟斤拷锟斤拷 */
							ptMsg->usData[2],              /* 锟斤拷锟斤拷锟斤拷锟斤拷 */
...
static void vTaskCOTL(void *pvParameters)
{
   ...
   // const TickType_t xFrequency = 200;
    xLastWakeTime = xTaskGetTickCount();
    BaseType_t xResult;
	const TickType_t xMaxBlockTime = pdMS_TO_TICKS(10); /* 锟斤拷锟斤拷锟斤拷锟饺达拷时锟斤拷为5ms */
	uint8_t ucControl=0;
